...
namespace LiveSplit.JumpKing {
    ...
    public partial class SplitterMemory {
        ...
        public float TotalGameTime() {
            //AchievementManager.instance.m_all_time_stats._ticks
            return IStatInfo.Read<int>(Program, 0x0, 0x3c) * 0.017f;
        }
        public int TimesWon() {
        ...
        //allTime = total / current = allTime - timestamp before attempt
        public int LiveJumps() {
            int allTime = IStatInfo.Read<int>(Program, 0x0, 0x28);
            int current = IStatInfo.Read<int>(Program, 0x0, 0xc);
            return allTime - current;
        }
        public int Jumps() {
            return IStatInfo.Read<int>(Program, 0x0, 0x28);
        }
        public int LiveFalls() {
            int allTime = IStatInfo.Read<int>(Program, 0x0, 0x2c);
            int current = IStatInfo.Read<int>(Program, 0x0, 0x10);
            return allTime - current;
        }
        public int Falls() {
            return IStatInfo.Read<int>(Program, 0x0, 0x2c);
        }
        public int Attempts() {
            return IStatInfo.Read<int>(Program, 0x0, 0x30);
        }
        public int Sessions() {
            int allTime = IStatInfo.Read<int>(Program, 0x0, 0x34);
            int current = IStatInfo.Read<int>(Program, 0x0, 0x18);
            return (allTime - current) + 1;
        }
